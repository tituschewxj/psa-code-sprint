            ...
            dcc.Graph(
                ...
                figure={
                    'data': [
                        ...
                        {
                            'type': 'scattermapbox',
                            'lat': [1.228645, 1.225964],
                            'lon': [103.616520, 103.620543],
                            'mode': 'markers',
                            'marker': {
                                'size': 15,
                                'symbol': 'circle',
...
def update_output(temp, humidity, weather, economic_growth, avg_fuel_price, contents, filename, n_clicks):
    if contents is None:
    ...
    mean_ships = calculate_mean_ships(temp, humidity, weather, economic_growth, avg_fuel_price)

    return f'Temperature is: {temp}°C, Humidity is: {humidity}%, Weather condition is: {weather}, World economic growth is: {economic_growth}%, Avg fuel price is: USD {avg_fuel_price}, Predicted Number of Ships is: {mean_ships}', data, columns
